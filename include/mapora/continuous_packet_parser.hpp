...
public:
  ...
  ContinuousPacketParser(const size_t count_clouds_to_extract_max);
...
private:
  ...
  size_t count_clouds_to_extract_max_;
  size_t  count_clouds_extracted_;
  ...
  std::vector<size_t> channel_to_ring_;
  ...
  Points cloud_;
  std::vector<Points> clouds_rings_;
  bool can_publish_again_;
  float angle_deg_cut_;

  bool skipped_cloud_first_;
};
}  // namespace mapora::points_provider::continuous_packet_parser
